...
const CarFavorites: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const recRes = await axios.get<AuctionPreview[]>(`http://localhost:5000/api/favorites/user-favorites/${userId}`);
    ...
    useEffect(() => {
        const fetchCars = async () => {
            try {
                const response = await axios.get<AuctionPreview[]>(`http://localhost:5000/api/favorites/user-favorites/${userId}`);
                ...
                <div style={{maxWidth, margin: "0 auto", padding: "16px" }}>
                    {loading ? (
                        ...
                        <div style={{ textAlign: "center", marginTop: "4rem" }}>
                            <IonText color="medium">
                                <h2>You haven’t parked any cars yet.</h2>
                                <p style={{ fontSize: "1rem" }}>
                                    Tap the “park” icon on a car to save it to favorites.
                                </p>
                            </IonText>
                        </div>
                    ) : (
